...
int BFSTree::C4exists(graph G)
{
	...
	for(i=0;(i<getVertexCnt()-1) && C4exist==0; ++i)
		for(j=i+1;j<getVertexCnt() && C4exist==0;++j)
		{
			missingEdge=isMissingEdge(G,i,j);
			//there is a missing egde between i and j
			if(missingEdge)
				//Some children of i has a missing Edge with some children of j
				for(k=0;k<getVertexCnt() && C4exist==0;++k)
					//k is child of i
					if(getAdjacency(i,k)==1)
						for(l=0;l<getVertexCnt() && C4exist==0;++l)
						{
							//l is child of j and exists missing Edge between k and l
							if(getAdjacency(j,l)==1 && isMissingEdge(G,k,l)==missingEdge)
							{
								C4exist=1;
							}
...
void BFSTree::listAllC3(graph G)
{
	int i,j,k;

	for(i=0;(i<getVertexCnt()-1); ++i)
		for(j=i+1;j<getVertexCnt();++j)
			if(isMissingEdge(G,i,j)==1)
			{
...
void BFSTree::listAllC4(graph G)
{

	int i,j,k,l,parent_i,parent_j;
	int missingEdge;
	...
	for(i=0;i<getVertexCnt()-1; ++i)
		for(j=i+1;j<getVertexCnt();++j)
		{
			...
			if(missingEdge)
			{
				//cout<<"Missing Edge "<<i<<" "<<j<<" "<<missingEdge<<endl;
				...
				if(missingEdge==2)
				{
					...
					if(parent(parent_i)==parent_j || parent_i==parent(parent_j))
					{
						cout<<"C4 Cycle : "<<i<<" "<<j<<" "<<parent_j<<" "<<parent_i<<endl;
					}
				}
				...
				for(k=0;k<getVertexCnt();++k)
				{
					...
					if(getAdjacency(i,k)==1)
					{
						for(l=0;l<getVertexCnt();++l)
						{
							//l is child of j and exists missing Edge between k and l
							if(getAdjacency(j,l)==1 && isMissingEdge(G,k,l)==missingEdge)
								cout<<"C4 Cycle : "<<i<<" "<<j<<" "<<k<<" "<<l<<endl;
